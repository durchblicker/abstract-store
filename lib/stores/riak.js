    ...
    res.on('end', function() {
      ...
      store.decompressor(dat)(dat, function(err, dat) {
        ...
        switch(dat.storage_type) {
          ...
          case 'buffer': return callback(null, new Buffer(dat.content, 'base64'));
...
function put(key, value, callback) {
  ...
  store.exec(url, value, function(res) {
    if (err) return callback(err);
...
function exec(url, data, callback) {
  ...
  Pea.first(this.options.host, function(host, callback) {
    ...
    url = Merge(url, this.options);
    url.protocol = this.options.secure ? 'https:' : 'http:';
    var request = (this.options.secure?HTTPS:HTTP).request(url, function(res) { callback(null, res); });
